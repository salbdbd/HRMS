using System.Collections.Generic;

namespace Hrms.ViewModels
{
   public class View_ExecutiveSalarySummaryViewModel
   {
      public string EmpCode { get;set; }
      public int PeriodID { get;set; }
      public string PeriodName { get;set; }
      public int CompanyID { get;set; }
      public decimal Basic { get;set; }
      public decimal DRN { get;set; }
      public decimal Mobl { get;set; }
      public decimal HRent { get;set; }
      public decimal ENT { get;set; }
      public decimal CONV { get;set; }
      public decimal MED { get;set; }
      public decimal GrossPay { get;set; }
      public decimal CALLOW { get;set; }
      public decimal PALOW { get;set; }
      public decimal LFA { get;set; }
      public decimal ELC { get;set; }
      public decimal SCOM { get;set; }
      public decimal ARRE { get;set; }
      public decimal AVD { get;set; }
      public decimal FB { get;set; }
      public decimal PB { get;set; }
      public decimal OALOW { get;set; }
      public decimal WAS { get;set; }
      public decimal EDU { get;set; }
      public decimal BEV { get;set; }
      public decimal DA { get;set; }
      public decimal INS { get;set; }
      public decimal TotalAddi { get;set; }
      public decimal Amount { get;set; }
      public decimal SW { get;set; }
      public decimal CO { get;set; }
      public decimal SWADV { get;set; }
      public decimal EPS { get;set; }
      public decimal PFADV { get;set; }
      public decimal LWP { get;set; }
      public decimal ODECT { get;set; }
      public decimal ASLY { get;set; }
      public decimal PFO { get;set; }
      public decimal ITAX { get;set; }
      public decimal HLON { get;set; }
      public decimal TotalDeduct { get;set; }
      public int TaxYearID { get;set; }
   }
}